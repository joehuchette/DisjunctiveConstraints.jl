...
function _build_base_model()
    ...
    x_v, x_c = MOI.add_constrained_variables(model, [IN(-1.0, 1.0) for _ in 1:2])
...
@testset "naive_big_m_formulation!" begin
    let method = DisjunctiveConstraints.NaiveBigM(DisjunctiveConstraints.IntervalArithmetic())
        ...
        sum_ci, lt_cis, gt_cis = @inferred DisjunctiveConstraints.naive_big_m_formulation!(model, method, disjunction)
